---
description: This page goes into detail about the various risks about the protocol
---
...
These are risks associated with the AI agents themselves.

### 1.1 AI Agent Misbehavior

AI agents are autonomous and make trading or operational decisions based on their inputs, model weights, and market data. There is always a possibility that:

* The AI misinterprets data or market signals
* The underlying model generates poor or irrational outputs
* External conditions cause the model to deviate from its trained intent

Even though EigenLayer AVS validation ensures that actions are authorized and verified, it cannot guarantee profitability or correct market judgment.

> AI agents may act consistently with their logic, yet still incur financial losses.

### 1.2 EOA Compromise (Phase 1)

In early deployments, AI agents use Externally Owned Accounts (EOAs) and private keys for on-chain execution. If an EOA key is compromised, leaked, or exploited, an attacker could:

* Drain the agent’s treasury
...
Risks related to code or smart contracts.
...
* Loss of funds
* Incorrect reward or airdrop distributions
* Stuck or inaccessible assets

Participants should recognize that no system is fully immune from unknown exploits or logic errors.
...
Risks associated with trading and general market.

### 3.1 AI Agents Running at a Loss

Each AI agent token has a treasury that trades and compounds autonomously. If its model performs poorly or encounters adverse market conditions:

* The agent’s treasury value may decline
* The token’s market price could fall sharply
* Holders could incur losses from speculation or poor agent performance

While 80% of trading fees are directed back to the treasury, these do not guarantee profitability especially in volatile or illiquid conditions.
